    ...
    public class TCPServer : IDisposable {
        ...
        private async Task ListenTo(Connection connection) {
            try {
                ...
                int receivedBytes = await socket.ReceiveAsync(new ArraySegment<byte>(connection.recvBuffer, 0, connection.recvBuffer.Length), SocketFlags.None).ConfigureAwait(false);
                ...
                if (receivedBytes > 0) {
                    ...
                    do {
                        ...
                        case Connection.State.waiting: {
                            connection.messageSize = BitHelper.ReadInt(new ArraySegment<byte>(connection.recvBuffer, 0, connection.recvBuffer.Length), ref bytesRead);
        ...
        private void Dispose(Connection connection) {
            acceptedConnections.TryRemove(connection.socket.RemoteEndPoint, out _);
            connection.Dispose();
        }
        ...
        public async Task<int> SendTo(byte[] data, EndPoint remoteEP) {
            if (acceptedConnections.TryGetValue(remoteEP, out Connection connection)) {
                ...
                try {
                    ...
                    int i = 0;
                    BitHelper.WriteBytes(new ArraySegment<byte>(data, 0, data.Length), new ArraySegment<byte>(connection.sendBuffer, 0, connection.sendBuffer.Length), ref i);
