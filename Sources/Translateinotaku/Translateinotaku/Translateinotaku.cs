...
public partial class Translateinotaku {
    private const string domain = "translatinotaku.net";
            ...
            using (HttpResponseMessage res = await client.SendAsync(request)) {
                if (res.IsSuccessStatusCode) {
                    using (HttpContent content = res.Content) {
                        ...
                        IElement title = document.QuerySelector("#chapter-heading")!;
                        state.epub.AppendLine($"<h1>{title.InnerHtml.Trim()}</h1>");
                        state.epub.AppendLine($"<p><a href=\"{url}\">Original</a></p>");

                        IElement biographia = document.QuerySelector(".reading-content .wp-biographia-container-around")!;
                        biographia.Remove();

                        IElement body = document.QuerySelector(".reading-content>div.text-left")!;
