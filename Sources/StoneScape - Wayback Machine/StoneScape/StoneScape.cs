...
public partial class StoneScape {
    private const string domain = "web.archive.org";
            ...
            using (HttpResponseMessage res = await client.SendAsync(request)) {
                if (res.IsSuccessStatusCode) {
                    using (HttpContent content = res.Content) {
                        ...
                        IElement title = document.QuerySelector(".breadcrumb>.active")!;
                        state.epub.AppendLine($"<h1>{title.TextContent.Trim()}</h1>");
                        state.epub.AppendLine($"<p><a href=\"{url}\">Original</a></p>");

                        IElement body = document.QuerySelector(".reading-content>.text-left")!;
