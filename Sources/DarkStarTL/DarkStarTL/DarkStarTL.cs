...
public partial class DarkStarTL {
    private const string domain = "darkstartranslations.com";
            ...
            using (HttpResponseMessage res = await client.SendAsync(request)) {
                if (res.IsSuccessStatusCode) {
                    using (HttpContent content = res.Content) {
                        ...
                        string vueJSON = document.GetElementById("app")!.GetAttribute("data-page")!;
                        JObject app = JObject.Parse(vueJSON);
                        JObject props = app.Value<JObject>("props")!;
                        JObject chapter = props.Value<JObject>("chapter")!;

                        state.epub.AppendLine($"<h1>{chapter.Value<string>("title")}</h1>");
                        state.epub.AppendLine($"<p><a href=\"{url}\">Original</a></p>");
                        state.epub.AppendLine($"<div class=\"content\">");

                        IElement body = parser.ParseDocument($"<html><head></head><body><div class='content'>{WebUtility.HtmlDecode(chapter.Value<string>("content"))}</div><body></html>").QuerySelector(".content")!;
