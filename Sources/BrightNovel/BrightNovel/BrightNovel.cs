...
public partial class BrightNovel {
    private const string domain = "brightnovels.com";
            ...
            using (HttpResponseMessage res = await client.SendAsync(request)) {
                if (res.IsSuccessStatusCode) {
                    using (HttpContent content = res.Content) {
                        ...
                        IElement data = document.QuerySelector("#app")!;
                        string json = WebUtility.HtmlDecode(data.GetAttribute("data-page")!);

                        JObject page = JObject.Parse(json);
                        JObject props = page.Value<JObject>("props")!;
                        JObject ziggy = props.Value<JObject>("ziggy")!;
