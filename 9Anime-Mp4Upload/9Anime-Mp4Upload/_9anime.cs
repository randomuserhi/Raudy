...
static class _9anime
{
    ...
    public static async Task<VideoEmbed?> GetEmbed(string episodeId)
    {
        _9anime.Response<_9anime.EncodedVideoEmbed>? req =
            await Rest.Get<_9anime.Response<_9anime.EncodedVideoEmbed>>
            ($"https://9anime.to/ajax/server/{episodeId}?vrf={GetVrf(episodeId)}");

        if (req is _9anime.Response<_9anime.EncodedVideoEmbed> resp)
        {
            if (resp.status == 200)
            {
                _9anime.EncodedVideoEmbed encoded = resp.result;
                _9anime.VideoEmbed embed = new _9anime.VideoEmbed();
                embed.url = DecodeVideoData(encoded.url);
                embed.skip_data = JsonSerializer.Deserialize<Dictionary<string, int[]>>(DecodeVideoData(encoded.skip_data));
                return embed;
            }
            else
            {
                Console.WriteLine($"Failed: {resp.status}");
            }
        }
